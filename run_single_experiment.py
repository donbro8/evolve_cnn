...
try:
    ...
    print(f"Starting single run of {experiment_type}.")
...
# exp_run = evolution.run_multiple_experiments(experiment_type = running_experiment)
    try:
        send_email_alert(subject=f"Experiment '{experiment_type}' completed", message=f"Experiment '{experiment_type}' completed successfully.")
    ...
    try:
        send_email_alert(subject=f"Experiment '{experiment_type}' failed", message=f"Exception: {e} \n {traceback_info}")
        print(f"Exception: {e} \n {traceback_info}")
    except Exception as e2:
        print("Warning: Email alert failed with exception:", e2)
        print(f"Exception: {e} \n {traceback_info}")