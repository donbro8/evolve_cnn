...
class BlockNEAT:
    ...
    def crossover(self, parents, population, path_to_minimal_individual):
        ...
        if np.sum(compatability_scores) > 0: # Otherwise parents are the same and crossover is pointless
    ...
    def offspring_proportion(self, population, max_species = 10):
        ...
        # Get the number of members in each species
        n_members = np.array([len(population_copy['species'][s]['members']) for s in species])

        # Number of species in the population
        n_species = len(species)

        # Number of possible species is restricted
        n_species_next_gen = np.min([n_species, max_species]) 

        # Calculate the sum of the shared fitness for each species
        ...
        # Set fitness to zero id there are two or less members in the species - need a way to get rid of species that aren't performing well
        fitness_species_sums = np.where(n_members <= 2, 0, fitness_species_sums)
        
        # If all species have the same fitness then sort by the number of members in each species
        if len(set(fitness_species_sums)) == 1: 
        ...
        # Otherwise sort by the fitness of each species
        else:
        ...
        species_next_gen = species[:n_species_next_gen]
        ...
        # len(list(combinations(members, n)))
        
        # Loop through all species
        for i in range(n_species):

            # if species[i] in species_next_gen:
    ...
    def evolve_block(self, X, y, save_path = os.getcwd() + '/data/evolution/', run = 1):
        ...
        for g in range(1, generations + 1):
            ...
            for species in subpopulation.keys():
                ...
                n_combinations = len(list(combinations(subpopulation[species]['members'], 2)))
